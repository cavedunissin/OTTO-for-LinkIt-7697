...
void MaxMatrix::writeMatrix(const uint8_t matrixData[8], MatrixRotation r) {
	switch(r){
	// identity
	case MATRIX_DIR_0:
		for (int r=0; r<8;r++){ 
		    for (int c=0; c<8; c++){
		        setDot(c,r, 0x1 & (matrixData[r] >> (7 - c)));
		    }
		}
		break;

	// rotate ccw 90 degree
	case MATRIX_DIR_270:
		for (int r=0; r<8;r++){ 
		    for (int c=0; c<8; c++){
		    	// transpose by swapping r,c
		    	// followed by reversing each column
		        setDot(r,c, 0x1 & (matrixData[r] >> (c)));
		    }
		}
		break;
	...
	case MATRIX_DIR_180:
		...
		for (int r=0; r<8;r++){ 
		    for (int c=0; c<8; c++){
		    	// flip y (by reversing row)
		    	// followed by reversing each column
		        setDot(c, 7 - r, 0x1 & (matrixData[r] >> (c)));
		    }
		}
		break;
	...
	case MATRIX_DIR_90:
		...
		for (int r=0; r<8;r++){ 
		    for (int c=0; c<8; c++){
		    	...
		    	// and then flip x
		        setDot(7-r, 7-c, 0x1 & (matrixData[r] >> (c)));
		    }
		}
		break;
	}
