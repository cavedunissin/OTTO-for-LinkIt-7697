...
class MaxMatrix
{
  ...
  public:
    ...
    void init(byte data, byte load, byte clock, byte num);
    void clearMatrix();
    ...
    // 8 rows x 8 colums. Each row is a uint8_t, each bit is a LED
    void writeMatrix(const uint8_t matrixData[8], MatrixRotation rotation = MATRIX_DIR_0);
};
